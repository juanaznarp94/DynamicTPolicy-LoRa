...
class loraEnv(Env):
    ...
    def __init__(self, N, BER):  # Method to initialize the attributes of the object we create
        ...
        self.packets_attempted = np.zeros(
            self.n)  # to store the sum of the transmissions attempted by the external nodes
    ...
    def step(self, action):
        ...
        if 1 == 1:
            ...
            print('BER(t): ' + str(self.ber))
            print('BER_tg: ' + str(self.ber_th))
        ...
        self.array_ber.append(self.ber)
        self.array_ber_th.append(self.ber_th)
        self.array_duration.append(self.duration)
        self.array_reward.append(reward)
        info = {}
        done = False
...
for i, b in enumerate(BER):
    ...
    ax[i].plot(env.array_reward, label="reward")
    ax[i].plot(env.array_duration, label="duration")
    ax[i].set_xticks([0, 1, 2, 3, 4, 5], ["SF7", "SF8", "SF9", "SF10", "SF11", "SF12"])
    ax[i].grid()
    ax[i].set_title('BER target: '+str(round(b,7)))
    plt.legend()
