...
class loraEnv(Env):
    ...
    def step(self, action):
        ...
        print('To_transmit_priorities: ' + str(priorities))

        to_transmit_priorities = np.multiply(to_transmit, priorities)
        print('To_total: ' + str(to_transmit_priorities))
        ...
        if 1==1:
        #if self.e > 0.8 * MAX_BATTERY_LEVEL:
            ...
            # PDR local
            pdr = np.sum(transmitted) / np.sum(to_transmit)
            print('PDR: ' + str(pdr))
            # PDR global
            ...
            #print('Sum_transmitted: ' + str(sum(transmitted)))

            # PRR
            ber = pow(10, alpha * math.exp(beta * snr))
            prr = (1 - ber) ** (PACKET_SIZE * sum(transmitted))
            print('PRR: ' + str(prr))
            ...
            # Calculate energy
            """
            payload = PACKET_SIZE * sum(transmitted)  # Bits
            print('Payload: ' + str(payload) + ' bits')
            h = 1
            de = 1
            n_p = 0  # lo incluimos en el payload
            n_payload = 8 + max((8 * payload - 4 * sf + 16 + 28 - 20 * h) / (sf - 2 * de), 0)
            t_symbol = pow(2, sf) / BW
            p_cons = 0.4125  # W

            e_bit = (p_cons * (n_payload + n_p + 4.25) * t_symbol) / (8 * payload)
            print('Energía consumida e_bit: ' + str(e_bit*1e6) + ' uJ')

            print('self.e 1 : ' + str(self.e) + ' J')
            self.e = self.e - e_bit * payload
            print('self.e 2 : ' + str(self.e) + ' J')"""
            ...
            # reward
            reward = 0.4 * heaviside(prr, 0.8) + 0.4 * heaviside(pdr, 0.8) - 0.2 * heaviside(e_pkt, 301.84456)
            print(reward)
