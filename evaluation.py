...
# Load model
model = PPO.load("lora_rl_sac_v0.zip")  # para cargar modelo que se haya generado en el solver
# TODO: Test other algorithms too

BER = [0.000138957548234, 6.390550739301948e-05, 2.4369646975025416e-05, 7.522516546093483e-06,
...
def step_csv(local_dir, i, ber_th, iterations):
    header = ['ber', 'ber_th', 'battery_life', 'prr', 'SF']
    with open(local_dir + str(i) + '.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
# PLOT
def plot_ber_comparison_bars(combine):
    # EVALUATE AND SAVE RESULTS
    local_dir = 'results/bars/'
    for i, ber_th in enumerate(BER_NORM):
        step_csv(local_dir, i, ber_th, 5)
    if combine:
    ...
    plt.tight_layout()
    plt.savefig(local_dir+'comparison_ber_bars.png', dpi=400)
    plt.show()


# PLOT
def plot_ber_comparison_lines(combine):
    # EVALUATE AND SAVE RESULTS
    local_dir = 'results/lines/'
    for i, ber_th in enumerate(BER_NORM):
        step_csv(local_dir, i, ber_th, 10)
    if combine:
        combine_csv_directory(local_dir)
    data = pd.read_csv(local_dir + 'total.csv')
    fig, ax1 = plt.subplots(1, figsize=(8, 5))
    sns.set_style("dark")
    sns.lineplot(x=data.index, y='ber', data=data, label="Measured BER", alpha=.5, color='red', ax=ax1)
    sns.lineplot(x=data.index, y='ber_th', data=data, label="Target BER", alpha=.5,
                color='blue', ax=ax1).set(xlabel='Uplink messages', ylabel='BER')
    plt.tight_layout()
    plt.legend()
    plt.savefig(local_dir+'comparison_ber_lines.png', dpi=400)
    plt.show()


combine = True
#plot_ber_comparison_bars(combine)
plot_ber_comparison_lines(combine)
